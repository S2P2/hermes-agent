    ...
    if adapter is not None:
        ...
        if info.get("type") == "dm":
            try:
                ...
                if session_chat_id:
                    session_info = await adapter.get_chat_info(session_chat_id)
                    if session_info.get("type") != "dm":
